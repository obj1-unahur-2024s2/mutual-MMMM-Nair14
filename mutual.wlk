...
class ViajesDePlaya inherits Actividades {
    const largo
...
class ExcursionACiudad inherits Actividades {
...
class SalidaDeTrekking inherits Actividades {
    const kilometrosDeSenderossARecorrer
    const diasDeSolPorAnio

    override method dias() = kilometrosDeSenderossARecorrer / 50
    method implicaEsfuerzo() = kilometrosDeSenderossARecorrer > 80
    override method sirveParaBroncearse() {
        return diasDeSolPorAnio > 200 || (diasDeSolPorAnio.between(100, 200)
               && kilometrosDeSenderossARecorrer > 120)
    }
...
class ClasesDeGimnasia inherits Actividades {
